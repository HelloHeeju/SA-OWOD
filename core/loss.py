...
class SetCriterionDynamicK(nn.Module):
    ...
    def forward(self, outputs, targets, disagreement=None):
        ...
        indices, _, ow_indices, unknown_targets = self.matcher(outputs_without_aux, targets, disagreement)
        ...
        if 'aux_outputs' in outputs:
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                indices, _, ow_indices, unknown_targets = self.matcher(aux_outputs, targets, disagreement)
                for loss in self.losses:
                    if loss == 'nc_labels':
                        if self.start_count > self.start_iter:
        ...
        with torch.no_grad():
            ...
            for batch_idx in range(bs):
                ...
                if self.cfg.MODEL.NC:
                    ...
                    if disagreement!=None and disagreement[batch_idx].has("pred_boxes"):
                        other_boxes = disagreement[batch_idx]._fields['pred_boxes'].tensor
                        
                        # IoU 기반 매칭으로 현재 모델의 RPN 공간으로 정렬
                        ious = ops.box_iou(bz_boxes, other_boxes)
                        max_iou, mapped_indices = ious.max(dim=0)
                        valid_mask = max_iou > 0.3
                        if valid_mask.any():
                            forward_index_from_disagreement = mapped_indices[valid_mask]
                        else:
                            forward_index_from_disagreement = None
                    else:
                        forward_index_from_disagreement = None
                    _, forward_index_from_score = torch.topk(forward_score, self.forward_k, largest=True, sorted=True)
                    
                    if forward_index_from_disagreement is None:
                        forward_index = forward_index_from_score
                    elif forward_index_from_score.numel() == 0:
                        forward_index = forward_index_from_disagreement
                    else:
                        forward_index = torch.cat(
                            (forward_index_from_score, forward_index_from_disagreement), dim=0
                        ).unique()
