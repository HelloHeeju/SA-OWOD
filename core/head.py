...
class DynamicHead(nn.Module):
    ...
    def forward(self, features, init_bboxes, t, init_features, roi_labels=None, samerpn=None):
        ...
        if samerpn is None:
            for head_idx, rcnn_head in enumerate(self.head_series):
                class_logits, objectness, pred_bboxes, proposal_features = rcnn_head(features, bboxes, proposal_features, self.box_pooler)
            ...
            for head_idx, rcnn_head in enumerate(self.head_series):
                class_logits, objectness, _, proposal_features = rcnn_head(features, bboxes, proposal_features, self.box_pooler)
                per_image_features   = torch.split(proposal_features, class_logits.shape[1], dim=1)
                proposal_features    = torch.cat(per_image_features, dim=0)
                pred_bboxes = init_bboxes                
                if self.return_intermediate:
                    inter_class_logits.append(class_logits)
                    inter_pred_bboxes.append(pred_bboxes)
                    inter_objectness.append(objectness)
                    inter_proposal_features.append(proposal_features)
                bboxes = init_bboxes.detach() 
